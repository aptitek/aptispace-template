...
## <i class="bi bi-hdd-fill"></i> Stockage en Ligne
...
## <i class="bi bi-layout-three-columns"></i> Stockage en Colonne
...
viewof queryCol = Inputs.radio(
  ["Aucune", "Âge", "Salaire"],
  { label: "Simuler une requête analytique CPU sur :", value: "Aucune" }
)
```
:::

::: {.card-body}
::: {#ram-graph-container style="height: 380px; position: relative; background: transparent; border-radius: 8px;"}
:::
...
// 2. Rendu réactif du graphe d'accès mémoire
renderRamGraph = {
  createRamStorageGraph("ram-graph-container", storageMode, queryCol);
}
```

::: {.card .card-info .mt-3 .mb-0}
::: {.card-header}
...
//| echo: false
{
  const isNone = queryCol === "Aucune";
  const isLigne = storageMode === "ligne";
  let title = "🔍 Mode d'accès au processeur";
  let content = "Sélectionnez une colonne ci-dessus pour observer le comportement du processeur lors de la lecture des données.";
  let alertClass = "is-debug";

  if (!isNone) {
    if (isLigne) {
      title = "⚠️ Cache Miss systématiques (Accès Ligne)";
      content = `Pour lire la colonne <strong>${queryCol}</strong>, le processeur doit sauter les autres attributs en RAM (ID, Salaire, etc.). Ces sauts non-contigus empêchent la prélecture matérielle, entraînant des <strong>Cache Miss</strong> répétés. Le CPU passe son temps à attendre la RAM.`;
      alertClass = "is-danger";
    } else {
      title = "⚡ Cache Hit et Vectorisation SIMD (Accès Colonne)";
      content = `Toutes les valeurs de la colonne <strong>${queryCol}</strong> sont stockées de manière strictement contiguë en mémoire. Le processeur charge le bloc entier dans ses caches L1/L2/L3 en une seule opération (<strong>Cache Hits</strong> massifs) et peut appliquer des instructions vectorielles <strong>SIMD</strong>. C'est le secret de la vitesse de Polars et Pandas moderne !`;
      alertClass = "is-success";
    }
  }

  return html`
    <div class="card ${alertClass} border-0 shadow-none mb-0">
      <div class="card-header border-0 pb-1" style="background: transparent; font-weight: 800;">${title}</div>
      <div class="card-body pt-1" style="background: transparent;">${content}</div>
    </div>
  `;
}
```
