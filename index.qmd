...
``````{ojs}
//| echo: false
import { renderStateMachine } from "./assets/js/networks.js"

testSmGraph = {
  const states = [
    { id: "A", label: "État A", val: "Initial", x: -80, y: 0 },
    { id: "B", label: "État B", val: "Traitement", x: 80, y: 0 }
  ];
  const transitions = [
    { source: "A", target: "B", label: "Transition" }
  ];

  const graph = renderStateMachine("test-sm-graph", states, transitions, {
    interval: 1100,
    loop: true,
    startBtnId: "btn-sm-start"
  });
